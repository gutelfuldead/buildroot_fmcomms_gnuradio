...
{
	const size_t nLines    = 24800;
	const size_t nChannels = 1;
	const int16_t amplitude = 2047;
	const size_t nSamples = nLines * nChannels;
	int16_t txI[nSamples];
	int16_t txQ[nSamples];
	int16_t txIleave[nSamples*2];
	...
	rxbuf = iio_device_create_buffer(rx, nSamples, false);
	if (!rxbuf) {
		perror("Could not create RX buffer");
		shutdown();
	}
	
	txbuf = iio_device_create_buffer(tx, nSamples, false);
	if (!txbuf) {
